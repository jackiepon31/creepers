					...
					<div class="col-md-12">
						<div class="widget wgreen">
							<div class="widget-head">
								<div class="pull-left">爬虫配置明细</div>
										...
										<div class="form-group">
											<table class="table table-striped table-bordered table-hover">
												<tbody>
													<tr>
														<td>爬虫任务类型:</td>
														<td><c:forEach items="${taskTypeList}" var="taskType"
																varStatus="index">
																<c:if test="${taskType.value == requestType}">
																	${taskType.name}
																	<input type="hidden" id="requestType"
																		name="requestType" value="${taskType.value}">
																</c:if>
															</c:forEach></td>
														<td>种子URL:</td>
														<td>${rootUrl}<input type="hidden" id="rootUrl"
															name="rootUrl" value="${rootUrl}"></td>
													</tr>
													<tr>
														<td>域名:</td>
														<td>${domain}<input type="hidden" id="domain"
															name="domain" value="${domain}"></td>
														<td>用户浏览器代理设置:</td>
